...
public class DemandResponsiveTransportationAccessShifter {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(DemandResponsiveTransportationAccessShifter.class);
  /**
  ...
  private static Result<Duration, Error> fetchArrivalDelay(
    ...
    if (DemandResponsiveTransportationAccessShifter.shouldShift(request, Instant.now())) {
      var shiftingResult = DemandResponsiveTransportationAccessShifter.arrivalDelay(request, services, now);
